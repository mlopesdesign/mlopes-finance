...
- **Sintoma reportado pelo Marcio** (noite de 13/08/2026, após importar `extrato_santander_janeiro_2026_IMPORTACAO.csv` pela segunda vez): banco tinha 126 `itens_importacao` (63 com `status='ignorado'` + 63 com `status='duplicado'`), 0 `lancamentos`, importacao #1 com `status='previa'`. Clicar "Confirmar importação" → erro `"Nenhum item pendente para importar."`.
- **Causa raiz** (2 bugs combinados):
  1. **`criarPreviaImportacao` só bloqueava reimportação se a anterior estava `status='confirmada'`**. Cenário: Marcio importou CSV, confirmou (status vira 'confirmada'), excluiu todos os 63 lançamentos via "Excluir todos" (que marca os 63 itens como 'ignorado' pela regra de lifecycle em `core/lancamentos.js:186`). Quando reimportou o mesmo CSV, a checagem antiga deixou passar porque a importacao anterior NÃO estava mais 'confirmada' (tinha sido 'consumida'). Resultado: 2ª importacao com 63 itens, todos marcados 'duplicado' (provavelmente pelo caminho de deteccao contra o que sobrou da 1ª importacao).
  2. **`excluirImportacao` confiava no `ON DELETE CASCADE` do schema**, mas **sql.js NÃO enforca FKs por padrão** (precisa `PRAGMA foreign_keys = ON`, que o app não habilita por questão de performance). Quando Marcio excluiu a 2ª importacao, os 63 itens dela ficaram órfãos no banco — `foreign_key_check` reportou 63 violações depois.
- **Fixes em `core/importacao.js`**:
  - `criarPreviaImportacao` agora bloqueia reimportação se existe QUALQUER importacao anterior (mesmo 'previa'/'cancelada') com o mesmo hash + contexto. Mensagem nova: `"Este arquivo ja tem uma importacao (#N, status=X) neste contexto. Exclua a anterior pelo historico para criar uma nova previa."` Pra forçar nova prévia, o user precisa excluir a anterior pelo botão "Excluir" do histórico.
  - `excluirImportacao` agora faz `DELETE FROM itens_importacao WHERE importacao_id = ?` MANUAL antes de deletar a importacao (não confia no CASCADE). Retorna também `itensRemovidos: N` no resultado.
  - **Nova função `reciclarImportacao(importacaoId)`**: marca TODOS os itens 'ignorado'/'duplicado' de volta como 'pendente' (NÃO mexe em 'importado' — tem lancamento vinculado). Retorna `{ ok, reciclados, status, mensagem }`. Caso de uso direto do Marcio: depois de excluir os lançamentos sem querer, ele pode reciclar a importacao e confirmar de novo sem perder o contexto de auditoria nem precisar re-fazer upload.
- **Nova rota no servidor** (`servidor.js`): `importacao:reciclar` e `importacao:contarPorStatus` (essa última retorna `{ pendente, importado, ignorado, duplicado }` pra UI decidir se mostra o botão "Reciclar").
- **UI** (`telas/importacao.js`): novo botão **"♻ Reciclar N"** no histórico de importações (aparece só quando há itens 'ignorado'/'duplicado' na linha da importacao). Confirmação simples: "Reciclar a importacao #N? Todos os itens 'ignorado'/'duplicado' voltam a 'pendente' e podem ser confirmados de novo.".
- **Bonus fix** (`core/lancamentos.js::excluirTodosLancamentos`): agora insere **1 linha de auditoria consolidada** (`acao='excluidosEmMassa'`, `dados_json={quantidade, ids}`) em vez de 63+ linhas individuais. Antes a tabela de auditoria ficava poluída com 1 linha por lançamento excluído.
- **6 testes novos** (91/91 verde): reimportacao bloqueada com anterior 'previa'/'cancelada' (v0.8.20), DELETE manual remove 100% dos itens mesmo com `PRAGMA foreign_keys=OFF`, reciclarImportacao muda 'ignorado'→'pendente' (3 testes cobrindo 100% ignorado, mix ignorado+duplicado+intocado, e 0 reciclaveis), contagem de itens por status para UI.
- **Limpeza do banco do Marcio**: rodei `tools/limpar-importacao-orfaos.py` que removeu 126 itens + importacao #1 + auditoria de importacoes. Mantidos: 1 contexto, 3 contas, 0 lancamentos, 0 importacoes. Banco pronto pra reimportar o CSV limpo.
- **Bump v0.8.19 → v0.8.20** (7 lugares). Bundle v0.8.20 (~5.99 MB) em publicacao no GH.
- **Motivação do user** (gravado pra nunca repetir): "difícil hein, vc consegue ou procuro outro agente?". Achar a causa raiz levou 1 turno porque o sintoma (63 ignorados + 63 duplicados) parecia uma contradicao do CHECK constraint — mas era o resultado de 2 imports separados com semanticas diferentes (1ª 'consumida' + 2ª duplicada), mais 63 órfãos de uma 3ª exclusão silenciosa.
